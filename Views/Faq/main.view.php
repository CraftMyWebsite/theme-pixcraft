...
<section class="py-8 lg:py-16 px-8 md:px-36 2xl:px-96">
    <div class="<?php if(ThemeModel::getInstance()->fetchConfigValue('faq_display_form')): {echo "lg:grid grid-cols-3 gap-6";} endif ?>">
        <?php if(ThemeModel::getInstance()->fetchConfigValue('faq_display_form')): ?>
            <div style="background-color: var(--bg-pixcraft-features); color: var(--color-pixcraft-features)" class="shadow-xl h-fit">
                <div class="page-title-divider text-center pt-1 w-full">
                    <h2 class="title-color font-semibold text-xl uppercase"><?= ThemeModel::getInstance()->fetchConfigValue('faq_question_title') ?></h2>
                </div>
                <div class="p-4">
        ...
        <div style="background-color: var(--bg-pixcraft-features); color: var(--color-pixcraft-features)" class="shadow-xl col-span-2 h-fit">
            <div class="page-title-divider text-center pt-1 w-full">
                <h2 class="title-color font-semibold text-xl uppercase"><?= ThemeModel::getInstance()->fetchConfigValue('faq_answer_title') ?></h2>
            </div>
            <div class="p-4">
                ...
                <div class="shadow-xl p-2 mb-4">
                    <div class="flex flex-wrap justify-between items-center mt-2">
                        ...
                        <?php if(ThemeModel::getInstance()->fetchConfigValue('faq_display_autor')): ?>
                            <p class="text-xs py-1 px-2 bg-gray-300"><?= $faq->getAuthor()->getPseudo() ?></p>
                        <?php endif; ?>
                    </div>
                    <p><?= $faq->getResponse() ?></p>
                </div>
